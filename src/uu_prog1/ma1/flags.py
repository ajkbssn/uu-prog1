...
def pentagram(x, y, side,
              fill_type=None, fill_color="black", line_color="black"):
...
def french_flag_with_stars(x, y, height):
    ...
    for i in range(2):
        for j in range(5):
            pentagram(
                x-height/4 + j*0.5*height, y-0.1*height+i*1.51*height,
                0.31*height, fill_type="outer",
                fill_color="dark green", line_color=""
                )


def vietnamese_flag(x, y, height):
    rectangle(x, y, 1.5*height, height, fill_color="red", line_color="")
    x_star = x + 0.8 * height
    y_star = y + 0.75 * height
    side = height * 6 / 10 * (10 ** (1/2) / (5 + 5 ** (1/2)) ** (1/2))
    pentagram(x_star, y_star, side,
              fill_type="full", fill_color="yellow", line_color="")
